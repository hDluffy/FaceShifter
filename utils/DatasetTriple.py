...
class FaceEmbed(TensorDataset):
    def __init__(self, data_path_list, same_prob=0.8):
        ...
        for data_path in data_path_list:
            image_list_A = glob.glob(f'{data_path}/train_A/*.*g')
            datasets_A.append(image_list_A)
            
            image_list_B = glob.glob(f'{data_path}/train_B/*.*g')
            datasets_B.append(image_list_B)
            
            image_list_M = glob.glob(f'{data_path}/train_M/*.*g')
            datasets_M.append(image_list_M)
    ...
    def __getitem__(self, item):
        ...
        # while item >= self.N[idx]:
            # item -= self.N[idx]
            # idx += 1
        image_path_M = self.datasets_M[idx][item]
        name = os.path.split(image_path_M)[1]
        # embed = self.embeds[idx][name]
        Xs = cv2.imread(image_path_M)
        Xs = Image.fromarray(Xs)
        
        image_path_A = self.datasets_A[idx][item]
        Xt = cv2.imread(image_path_A)
        Xt = Image.fromarray(Xt)
        
        image_path_B = self.datasets_B[idx][item]
        Xr = cv2.imread(image_path_B)
        ...
        # else:
            # Xt = Xs.copy()
            # same_person = 1
        return self.transforms(Xs), self.transforms(Xt), self.transforms(Xr)

    def __len__(self):
        return len(self.datasets_M[0])
